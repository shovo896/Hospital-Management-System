...
function showRegisterForm() {
    ...
    document.getElementById('register-role-title').textContent =
        titles[selectedRoleForAuth];

    const employeeIdField = document.getElementById('employee-id-field');
    const departmentField = document.getElementById('department-field');
    const passwordRegisterField = document.getElementById(
...
function updateNavigationForRole() {
    ...
    if (currentRole) {
        ...
        document.getElementById(navButtons[currentRole]).style.display =
            'block';
    } else {
...
function showToast(message) {
    ...
    toast.style.cssText =
        'position:fixed;top:20px;right:20px;background:#1e40af;color:white;padding:15px 25px;border-radius:8px;z-index:10000;';
...
function showPatientTab(tabId) {
    document
        .querySelectorAll('#patient-dashboard .dashboard-tab')
        .forEach(tab => {
...
function showDonorTab(tabId) {
    document
        .querySelectorAll('#donor-dashboard .dashboard-tab')
        .forEach(tab => {
            tab.classList.remove('active');
        });
...
function showEmployeeTab(tabId) {
    document
        .querySelectorAll('#employee-dashboard .dashboard-tab')
        .forEach(tab => {
            tab.classList.remove('active');
        });
...
function loadPatientProfile() {
    ...
    document.getElementById('view-blood').textContent =
        currentUser.bloodGroup || 'N/A';
    document.getElementById('view-registered').textContent =
        formatDateTime(currentUser.registrationDate);
}
    ...
    hospitalData.doctors.forEach(doctor => {
        ...
        card.innerHTML = `
            ...
            <div class="doctor-info">
                ...
                <div class="doctor-contact">Phone: ${doctor.phone}</div>
                <div class="doctor-fee">Fee: ৳${doctor.fee}</div>
            </div>
        `;
...
function loadPatientAppointments() {
    ...
    const myApts = hospitalData.appointments.filter(
    ...
    if (myApts.length === 0) {
        ...
        div.textContent = 'No appointments';
...
function toggleProfileEdit() {
    ...
    if (edit.style.display === 'none' || edit.style.display === '') {
        ...
        document.getElementById('edit-blood').value =
...
function cancelProfileEdit() {
    ...
    document.getElementById('edit-profile-btn').textContent =
        '✏️ Edit Profile';
}

function loadDonorDashboard() {
    if (!currentUser) return;
    // Future donor-specific loading
}

function loadEmployeeDashboard() {
    if (!currentUser) return;
    // Future employee-specific loading
}
...
function bookDiagnostic(type) {
    ...
    document
        .getElementById('diagnostic-modal')
        .classList.add('active');
}
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', function (e) {
            ...
            if (selectedRoleForAuth === 'admin') {
                ...
                user = userData.employees.find(
                    e => e.phone === username && e.password === password
                );
                if (user) authenticated = true;
            }
